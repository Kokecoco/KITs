...
$webButton.addEventListener('click', () => {mode = changeMode('web');});
$journalButton.addEventListener('click', () => {mode = changeMode('journal');});
$bookButton.addEventListener('click', () => {mode = changeMode('book');});
document.getElementById('create-button').addEventListener('click', () => {
  const webNeeds = [inputData.pageName, inputData.url, inputData.access.year, inputData.access.month, inputData.access.day];
  const journalNeeds = [inputData.author, inputData.journal, inputData.title, inputData.volume, inputData.pageCount, inputData.publish.year];
  const bookNeeds = [inputData.author, inputData.publisher, inputData.title, inputData.publish.year];

  if (mode !== 'web') { inputData.publish.month.value = ''; inputData.publish.day.value = ''; }
...
function changeMode(mode) {
  ...
  document.getElementsByName('form-block').forEach(
    block => {
      if (inputBlocks[mode].includes(block.id)) block.style.setProperty('display', 'block');
      else block.style.setProperty('display', 'none');
    }
  )

  const modeButton = {'web': $webButton, 'journal': $journalButton, 'book': $bookButton};
  ...
  if (mode === 'web') { 
    ...
    if (mode === 'journal') { $detailRequired.style.setProperty('display', 'inline'); $titleLabel.innerHTML = '論文タイトル <span class="required">*</span>'; }
    if (mode === 'book') { $detailRequired.style.setProperty('display', 'none'); $titleLabel.innerHTML = '書籍名 <span class="required">*</span>'; }
  }
  return mode;
}
...
function createAPA(needs) {
  // 必須事項の確認
  if (needs.filter( need => !need.value ).length) {alert('必須事項を入力してください'); return;}
  ...
  switch (mode) {
    ...
    case 'journal':
      common = `${begin}. ${inputData.title.value}. <i>${inputData.journal.value}, ${inputData.volume.value}</i>, ${inputData.pageCount.value}`;
    ...
    case 'book':
      ...
      if (edition && url) result = `${common}(${edition}). ${publisherName}. ${url}`;
